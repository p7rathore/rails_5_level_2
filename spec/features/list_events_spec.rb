...
describe "Viewing the list of events" do 
	it "shows the event" do 
		...
		expect(page).to have_text(event2.description[1..10])
		expect(page).to have_text(event2.starts_at)
		expect(page).to have_text(event2.price)
		
		expect(page).to have_text(event3.name)
		expect(page).to have_text(event3.location)
		expect(page).to have_text(event3.description[1..10])
		expect(page).to have_text(event3.starts_at)
		expect(page).to have_text(event3.price)
	end	
end